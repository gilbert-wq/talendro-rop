...
interface KanbanBoardProps {
  /** Scopes the board to a single requirement's submissions — used by
   * RequirementPipelineDialog (FG ID click → per-JD pipeline). Omit for the
   * global, all-requirements board (KanbanPage). */
  requirementId?: string
  compact?: boolean
}

/** Shared Kanban board logic (fetch, drag/drop status updates, rendering),
 * extracted out of KanbanPage so it can be reused scoped to a single
 * requirement inside RequirementPipelineDialog without duplicating the
 * drag-and-drop implementation. */
export function KanbanBoard({ requirementId, compact }: KanbanBoardProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={cn("flex gap-3 overflow-x-auto pb-4", compact && "max-h-[60vh]")}>
        {STAGES.map(stage => {
                  ...
                  >
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {!requirementId && <p className="text-xs text-muted-foreground truncate mono">{card.fg_id}</p>}
                      </div>
                    </div>

                    <div className="space-y-1">
                      {!requirementId && <p className="text-xs text-muted-foreground truncate">{card.requirement_title}</p>}
